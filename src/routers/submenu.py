...
async def update_submenu(menu_id: UUID, id: UUID, data: MenuUpdateSchema):
    ...
    return await submenu_services.update(data, id, menu_id)

@router.delete('/{id}')
async def delete_submenu(menu_id: UUID, id: UUID):
    submenu_services = SubmenuServices()
    return await submenu_services.delete(id)
